...
exports.executeFeedback = function(feedback, bank) {
	...
	if (feedback.type === 'varMatchString') {
		switch (opt.func) {
			case 'gt':
				if (parseFloat(variable[opt.varSelect].value) >= opt.value) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
			case 'lt':
				if (parseFloat(variable[opt.varSelect].value) <= opt.value) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
			case 'ne':
				if (variable[opt.varSelect].value != opt.value) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
			default:
				if (variable[opt.varSelect].value == opt.value) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
	...
	if (feedback.type === 'varMatchVar') {
		...
		switch (opt.func) {
			case 'gt':
				if (parseFloat(variable[opt.varSelect].value) >= temp) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
			case 'lt':
				if (parseFloat(variable[opt.varSelect].value) <= temp) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
			case 'ne':
				if (variable[opt.varSelect].value != temp) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
			default:
				if (variable[opt.varSelect].value == temp) { return { color: feedback.options.fg, bgcolor: feedback.options.bg1 }; }
				break
